...
class VertexElementIndexer<const CGAL::Point_2<K>> {
 public:
  auto operator()(const Kernel::Point_2& point, size_t coordinateIndex)
      -> decltype(std::declval<Kernel::Point_2>()[0]) const {
    ...
    return (coordinateIndex < 2)
               ...
               : static_cast<decltype(std::declval<Kernel::Point_2>()[0])>(0);
  }
};
